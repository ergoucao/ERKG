...
class Encoder(nn.Module):
    ...
    def __init__(self, layers, statusLayers):
        super().__init__()
        self.layers = nn.ModuleList(layers)
        self.statusLayers = nn.ModuleList(statusLayers)
        self.fuse_layer = nn.ModuleList(
            nn.Conv1d(in_channels=512*2,out_channels=512, kernel_size=1)
            for _ in range(len(self.layers))
        )

    def forward(self, res, level, attn_mask=None, status_emb=None):
        growths = []
        seasons = []
        season_attns = []
        growth_attns = []
        for idx,layer in enumerate(self.layers):
            ...
            if (status_emb!=None):
                _, _, status_growth, status_season, _, _ = self.statusLayers[idx](status_emb, level, attn_mask=None)
                growth = self.fuse_layer[idx](torch.cat([status_growth,growth],dim=2).transpose(dim0=1,dim1=2)).transpose(dim0=1,dim1=2)
                season = self.fuse_layer[idx](torch.cat([status_season,season],dim=2).transpose(dim0=1,dim1=2)).transpose(dim0=1,dim1=2)
